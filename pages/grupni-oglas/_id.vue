                ...
                <div class="listing-content-wrapper">
                    <div class="article-title">
                        ...
                        <div class="buttons" v-if="$auth.user">
                            ...
                            <button v-if="listing.user.id !== $auth.user.id" @click="toggleSaveListing"
                                    :class="listingSaved? 'listing-saved' : ''">
                    ...
                    <!--          </div>-->
                    <SingleQuestion v-if="questions.length" v-for="question in questions" :message="question"
                                    :key="question.id" :owner="owner"></SingleQuestion>
                    <div class="question-create"
                         v-if="$auth.user && listing.questions_disabled === false && owner === false">
                        <textarea v-model="questionTerm"></textarea>
                        <ActionButton placeholder="Postavi pitanje" @action="askQuestion"></ActionButton>
                    </div>
                </div>
                <div class="user-wrap">
                    <UserProfile :listing="listing" :user="listing.user" :followed="isFollowed"
                                 :is-rent="listing.is_rent" :type="listing.user.user_type"></UserProfile>
                </div>
            </div>
        </div>
    </div>
    ...
    async toggleSaveListing() {
        try {
            if (!this.listingSaved) {
                ...
                this.$toast.open({
                    message: "Uspješno ste snimili oglas " + this.listing.title,
                    type: 'success',
                    duration: 5000
                });
                ...
                this.$toast.open({
                    message: "Uspješno ste izbrisali oglas " + this.user.name + "iz spasenih",
                    type: 'success',
                    duration: 5000
                });
