        ...
        <div class="content-wrapper w-full" v-if="listingLoaded">
            <div class="step-1">
                ...
                <div class="grid-filters">
                    ...
                    <TextField type="text" label="Adresa" v-model="listing.address"
                               @input.native="showAddressAutocomplete"></TextField>
                    <ul v-if="recommendedAddresses.length">
                        <li v-for="item in recommendedAddresses" :key="item.id"
                            @click="address = item.description; recommendedAddresses = []">
                            {{ item.description }}
                        </li>
                    </ul>
                ...
                <div class="bg-50 flex flex-col w-full mt-6">
                    <div class="flex flex-row items-end discount-button">
                        <TextField type="number" label="Popust na cijenu (akcija)" placeholder="npr. 90"
                                   v-model="discount" :discount="true"></TextField>
            ...
            <div class="w-full my-10 flex-col">
                ...
                <div
                    class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md mb-10">
                    <label for="upload" class="space-y-1 text-center">
                        ...
                        <div class="flex flex-col text-sm text-gray-600">
                            <label class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600">
                                ...
                                <input type="file" class="hidden" accept="image/*" @change="uploadImages" multiple
                                       ref="upload">
                            </label>
            ...
            <ActionButton @action="saveChanges"
                          :style-options="{ background: 'transparent', border: '2px solid #1F2937', color: '#1F2937', borderRadius: '6px', minHeight: '48px', height: '48px', marginRight: '24px', fontSize: '13px', width: $device.isMobile ? '100%' : 'auto', paddingBottom: $device.isMobile ? 'pb-8' : 'auto'}"
                          class="mt-6" placeholder="Spasi izmjene"></ActionButton>
        </div>
    </div>
</template>
