...
export default class urediProfil extends Vue {
  ...
  async updateBanner(event) {
    if (event.target.files.length) {
      ...
      try {
        await this.$axios.post('/profile/agencija/banner', formData, {
          'headers': {
  ...
  async fetchMyAgency() {
    try {
      this.agency = (await this.$axios.get('/profile/agencija')).data.data;
    } catch (e) {
      console.log(e)
    }
  }
  ...
  async updateProfileInfo() {
    ...
    try {
      ...
      await this.$axios.put('/profile/agencija', payload)
