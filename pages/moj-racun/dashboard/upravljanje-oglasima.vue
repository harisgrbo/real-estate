    ...
    <div class="upravljanje-wrapper w-full" v-if="initialListingsLoaded">
        <div class="grid-cols-5 grid gap-4 mobile-grid" v-if="listings.length">
            <ListingCard v-for="listing in listings" :listing="listing" :from="true"
                         @remove-listing="handleRemoveListingModal(listing.id)"
                         @finish-listing="handleFinishListing($event)" @edit-listing="handleEditListing($event)"
        ...
        <modal @before-open="beforeOpen" @before-close="beforeClose" name="delete-listing" :adaptive="true"
            ...
            <div class="modal-inner">
                ...
                <div class="modal-content">
                    <h3 class="text-md">Jeste li sigurni da želite izbrisati oglas? Izbrisan oglas nije moguće
                        vratiti</h3>
                    ...
                    <div class="w-full items-center flex flex-row">
                        <action-button
                            :style-options="{ color: '#fff', width: '100%', background: 'red', marginRight: '6px' }"
                            class="mt-4" placeholder="Da" @action="handleRemoveListing"
                            :loading="loading"></action-button>
                        <action-button :style-options="{ color: '#fff', width: '100%', marginLeft: '6px' }" class="mt-4"
                                       placeholder="Ne" @action="$modal.hide('delete-listing')"
                                       :loading="loading"></action-button>
                    </div>
                </div>
            </div>
