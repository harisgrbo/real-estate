<template>
    <div class="preview-wrapper-inner mx-auto w-full">
        ...
        <div class="content w-full">
            <ul class="main-tabs">
                <li @click="currentTab = index" :class="[ currentTab === index ? 'active' : '' ]"
            ...
            <div v-show="currentTab === 0" class="w-full">
                <div v-if="listingsLoaded">
                    <div class="grid-cards" v-if="listings.length">
                        <ListingCard v-for="listing in listings" :listing="listing" :from="true"
                                     @remove-listing="handleRemoveListingModal($event)"
                                     @finish-listing="handleFinishListing($event)"
                                     @edit-listing="handleEditListing($event)" action_text="Opcije oglasa"
                                     :key="listing.id"/>
                    </div>
            ...
            <div v-show="currentTab === 1" class="w-full">
                <div v-if="completedListingsLoaded">
                    <div class="grid-cards" v-if="completed_listings.length">
                        <ListingCard v-for="listing in completed_listings" :listing="listing" :from="true"
                                     @remove-listing="handleRemoveListingModal($event)"
                                     @finish-listing="handleFinishListing($event)"
                                     @edit-listing="handleEditListing($event)" action_text="Opcije oglasa"
                                     :key="listing.id"/>
        ...
        <modal @before-open="beforeOpen" @before-close="beforeClose" name="delete-listing" :adaptive="true"
               height="100%">
            <div class="modal-inner">
                ...
                <div class="modal-content">
                    ...
                    <div class="w-full items-center flex flex-row">
                        <action-button
                            :style-options="{ color: '#1F2937', width: '100%', background: 'transparent', marginRight: '6px', border: '2px solid #1F2937' }"
                            class="mt-4" placeholder="Da" @action="handleRemoveListing"
                            :loading="loading"></action-button>
                        <action-button
                            :style-options="{ color: '#1F2937', width: '100%', background: 'transparent', marginLeft: '6px', border: '2px solid #1F2937' }"
                            class="mt-4" placeholder="Ne" @action="$modal.hide('delete-listing')"
                            :loading="loading"></action-button>
                    </div>
                </div>
            </div>
