<template>
    <div class="relative">
        ...
        <div class="mt-1 relative rounded-md input-wrap">
            ...
            <input :type="type"
                   :value="value"
                   @input="updateValue($event.target.value)" @blur="$emit('blur')" :placeholder="title" name="price"
                   id="price"
                   :class="['block w-full pl-4 pr-24 sm:text-sm rounded-md', error && error.length ? 'border-red-300 text-red-900 placeholder-red-300' : '']"
