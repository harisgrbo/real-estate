  ...
  <fieldset>
    <div class="mt-4 grid grid-cols-1 gap-y-6 sm:gap-x-4">
      ...
      <label class="relative bg-white border rounded-lg shadow-sm p-4 flex cursor-pointer focus:outline-none"
             v-for="option in options" :class="[value && value.id === option.id? 'ring-2 ring-gray-900 border-transparent': 'border-gray-300']" :key="option.id" @change="selectOption(option)"
      >
        <input
        ...
        -->
        <svg v-show="value && value.id === option.id" class="h-5 w-5 text-gray-900" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
