<template>
    <div class="listing-card-wrapper" :class="[from? 'blured' : '', 'sponsored-' + listing.sponsored]">
        ...
        <div v-if="!$device.isMobile" class="blured-background" @mouseover="showListingOptions = true"
             @mouseleave="showListingOptions = false" @click.stop>
            <div
                v-show="showListingOptions && ($router.history.current.fullPath === '/moj-racun/moji-oglasi' || $router.history.current.fullPath === '/moj-racun/dashboard/upravljanje-oglasima')"
                class="w-full">
                <action-button class="option-btn" placeholder="Uredi oglas" :style-options="{ width: '100%'}"
                               @action="$router.push('/oglas/uredjivanje/' + listing.id)"></action-button>
        ...
        <nuxt-link
            :to="this.$route.fullPath !== '/moj-racun/dashboard/grupisanje-oglasa'? '/oglas/' + listing.slug : '' ">
                ...
                <div class="flex flex-col justify-between items-start">
                    ...
                    <div class="icons-date flex flex-row items-center justify-between w-full" v-if="listing.price != 0">
                        ...
                        <div class="important" v-if="listing.hasOwnProperty('discount')">
                            ...
                            <p v-show="listing.is_booking" class="pl-2">/ noć {{
                                    listing.per_guest ? 'po osobi' : ''
                                }}</p>
                        </div>
                    </div>
