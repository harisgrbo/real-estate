<template>
    <div
        :class="['navbar-wrapper w-full lg:shadow-sm xl:shadow-sm up:shadow-sm sm:shadow-none max-w-full', this.$route.name === 'index' ? 'only-index' : '']">
        <div
            ...
            <div v-if="!$device.isMobile" :class="['flex items-center justify-center flex-1 centralize']">
                <div class="input-wrapper"
                     ...
                     v-on-clickaway="away"
                >
                    <svg @click="search" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
          ...
          <span class="relative z-0 inline-flex ml-2 rounded-sm border border-gray-200 selected-cat-type"
            ...
            <div @click="selectedCategory = null" type="button"
                 class="-ml-px cursor-pointer relative inline-flex items-center px-1 py-1 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24"
                ...
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        ...
                        <span class="relative z-0 ml-2 inline-flex rounded-sm border border-gray-800 selected-cat-type"
                              v-if="selectedType !== null">
            <div type="button"
                 class="relative inline-flex items-center px-1 py-1 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
              {{ selectedType.title }}
            </div>
            <div @click="selectedType = null" type="button"
                 class="-ml-px cursor-pointer relative inline-flex items-center px-1 py-1 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24"
                   stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
...
<!--                    </button>-->
                    ...
                    <div v-if="$auth.user" class="login-wrapper rounded-full cursor-pointer"
                         @click="showUserDropdown = !showUserDropdown">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        <span class="flex items-center pl-2 py-2">
              {{ $auth.user.name }}
            </span>
                        <button class="bg-transparent hover focus:outline-none">
                            <svg xmlns="http://www.w3.org/2000/svg"
                                 :class="['h-4 w-4', showUserDropdown ? 'transform rotate-180' : '']" fill="none"
                                 viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M19 9l-7 7-7-7"/>
                            </svg>
                    ...
                    <client-only>
                        <div class="user-dropdown" v-if="showNotifications" v-on-clickaway="handleCloseNotifications">
                            <NotificationsDropdown @clicked="$modal.hide('notifications')"
                                                   :notifications="notifications"
                                                   @close-notifications="handleCloseNotifications"
        ...
        <client-only>
            <modal @before-open="beforeOpen" @before-close="beforeClose" name="notifications" :adaptive="true"
                   height="100%">
                <NotificationsDropdown @clicked="$modal.hide('notifications')" :notifications="notifications"
                                       @close-notifications="$modal.hide('notifications')"
                                       @clear-notifications="handleClearNotifications"></NotificationsDropdown>
            </modal>
        </client-only>
    </div>
...
import {buildCategory, buildTitle, buildType} from '@/util/search'
