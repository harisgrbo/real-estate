            ...
            <div class="listing-card-content p-2 relative">
                <div class="flex flex-col justify-between items-start">
                    ...
                    <div class="icons-date flex flex-row items-center justify-between w-full">
                        <div class="important">
                            <p :class="['new', listing.hasDiscount ? 'cross' : '']">
                                {{ parseInt(listing.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") }} KM</p>
                            <p v-show="listing.listing_type === 'Stan na dan' && !listing.hasOwnProperty('discount')"
                               class="pl-2">/ noć {{ listing.per_guest ? 'po osobi' : '' }}</p>
                        </div>
